...
const findContainerId = node => {
  const cmd = `
    docker ps \
      ...
      -q
  `
  const containerId = execa(cmd, { shell: true }).stdout.toString('utf-8')
...
const createTopic = (containerId, topicName) => {
  ...
  return execa(cmd, { shell: true }).stdout.toString('utf-8')
}
...
console.log(
  execa
    (`docker-compose -f ${process.env.COMPOSE_FILE} ps`, { shell: true })
